...
public class NfcUtils {
     ...
     *
     * @param network the Wi-Fi configuration to write to the tag
     * @param tag the tag to write
     * @return true if the configuration has successfully been written to the tag, false otherwise
     */
    public static boolean writeTag(Network network, Tag tag) {
        return NfcUtils.writeTag(generateNdefMessage(network), tag);
    }
     ...
     *
     * @param network the Wi-Fi configuration to convert
     * @return an NDEF message containing the given Wi-Fi configuration
     */
    public static NdefMessage generateNdefMessage(Network network) {
        byte[] payload = generateNdefPayload(network);
    ...
    private static byte[] generateNdefPayload(Network network) {
        String ssid = network.getSsid();
        short ssidSize = (short) ssid.getBytes().length;

        short authType;
        switch (network.getAuthType()) {
            case WPA_PSK:
